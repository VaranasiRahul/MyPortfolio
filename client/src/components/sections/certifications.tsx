...
const achievements = [
  ...
  {
    title: "DSA & MySQL Certification",
    description: "Certified by NSDC & GDG MAD",
    icon: <Code className="w-5 h-5 text-indigo-400" />
  },
  {
    title: "Content Creator",
    description: "Administrator of 'RR REALMS' YouTube channel with 1.4k subscribers",
          ...
          <div>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              {achievements.map((item, index) => (
                <Card key={index} className="bg-card/30 border-border/50 p-5 flex gap-4 items-center hover-elevate">
                  <div className="w-10 h-10 rounded-full bg-background flex items-center justify-center shrink-0 border">
                    {item.icon}
                  </div>
