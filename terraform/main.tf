...
resource "aws_instance" "jenkins_ci" {
  ...
  user_data = templatefile("${path.module}/user-data-ci.sh", {
...
resource "aws_ssm_parameter" "sonarcloud_token" {
  name  = "/${var.project_name}/sonarcloud/token"
  type  = "SecureString"
